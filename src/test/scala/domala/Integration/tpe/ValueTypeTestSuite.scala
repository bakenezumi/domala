...
class ValueTypeTestSuite  extends FunSuite with BeforeAndAfter {
  ...
  test("select basic value type") {
    Required {
      assert(dao.selectBasic(0) === ValueTypeBasic(0, false, 0: Short, 0, 0, 0.0f, 0.0))
    }
  }

  test("insert basic value type") {
    Required {
      dao.insertBasic(ValueTypeBasic(1, true, 1: Short, 1, 1, 1.0f, 1.0))
      assert(dao.selectBasic(1) === ValueTypeBasic(1, true, 1: Short, 1, 1, 1.0f, 1.0))
    }
  }

  test("select basic value type option") {
    Required {
      assert(dao.selectOption(0) === ValueTypeOption(0, Some(false), Some(0: Short), Some(0), Some(0), Some(0.0f), Some(0.0)))
    }
  }

  test("insert basic value type option") {
    Required {
      val entity = ValueTypeOption(1, Some(true), Some(1: Short), Some(1), Some(1), Some(1.0f), Some(1.0))
  ...
  test("insert basic value type none") {
    Required {
      val entity = ValueTypeOption(1, None, None, None, None, None, None)
      dao.insertOption(entity)
      assert(dao.selectOption(1) === entity)
    }
  }
...
insert into value_types (id, boolean, short, int, long, float, double) values(0, false, 0, 0, 0, 0, 0);
    """)
  def create()
