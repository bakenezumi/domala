...
object DomaType {
  ...
  case class Basic(originalType: Type, convertedType: Type, wrapperSupplier: Term.Function, isNumeric: Boolean = false) extends DomaType
  case class EntityOrHolderOrEmbeddable(tpe: Type) extends DomaType
  case object UnSupport extends DomaType

  def of(tpe: Type.Arg): DomaType = {
    ...
    tpe match {
      ...
      case t"Seq[$elementTpe]" => DomaType.Seq(of(elementTpe), elementTpe)
      case t"Option[$elementTpe]" => DomaType.Option(of(elementTpe), elementTpe)
      case t"Optional[$elementTpe]" => DomaType.Option(of(elementTpe), elementTpe)
  ...
  def ofEntity(tpe: Type.Arg): DomaType = DomaType.of(tpe) match {
