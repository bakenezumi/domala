...
class TypeDeclaration[C <: blackbox.Context](c: C)(
  ...
  val typeParameterDeclarationsMap: mutable.Map[String, Seq[TypeParameterDeclaration[C]]] = TypeDeclaration.gatherTypeParameterDeclarations(c)(tpe)
  ...
  def isSameType(other: TypeDeclaration[C]): Boolean = tpe =:= other.tpe ||
    (this.isNumberType && other.isNumberType && this.numberPriority == other.numberPriority)
  def isTextType: Boolean = tpe <:< c.typeOf[String] || tpe <:< c.typeOf[Char] || tpe <:< c.typeOf[Character]
  ...
  def emulateArithmeticOperation(other: TypeDeclaration[C]): TypeDeclaration[C] = {
    assertNotNull(other, "")
    assertTrue(isNumberType)
    assertTrue(other.isNumberType)
    val tpe: C#Type = if (this.numberPriority.get >= other.numberPriority.get) this.tpe
    else other.tpe
    ...
    for ((binaryName, typeParameterDeclarations) <- typeParameterDeclarationsMap) {
      ...
      typeElement.foreach(e =>
        for (method: MethodSymbol <- e.members.collect {
          case m: MethodSymbol if m.isPublic && m.name.toString == name  && !(m.returnType =:= c.typeOf[Unit]) => m
        }){
          val parameters = method.paramLists.flatten.map(_.typeSignature)
          if (parameters.size == parameterTypeDeclarations.size) {
            if (parameterTypeDeclarations.map(_.getType).zip(parameters.map(p => resolveTypeParameter(p, typeParameterDeclarations))).forall { t =>
              t._1 <:< t._2
            }) {
              val methodDeclaration = new MethodDeclaration(c)(method, typeParameterDeclarations)
  ...
  def getFieldDeclaration(name: String): FieldDeclaration[C] = getFieldDeclarationInternal(name, false)
  def getStaticFieldDeclaration(name: String): FieldDeclaration[C] = getFieldDeclarationInternal(name, true)

  def getFieldDeclarationInternal(name: String, statik: Boolean): FieldDeclaration[C] = {
    ...
    for ((typeQualifiedName, typeParameterDeclarations) <- typeParameterDeclarationsMap) {
      ...
      typeElement.foreach(e =>
        for (field: TermSymbol <- e.members.collect {
          case f: TermSymbol if !(statik && !f.isStatic) && f.name.toString == name  && !f.isMethod=> f
        }) {
          val fieldDeclaration = new FieldDeclaration(c)(field, typeParameterDeclarations)
...
object TypeDeclaration {
  def newBooleanTypeDeclaration[C <: blackbox.Context](c: C): TypeDeclaration[C] = {
    new TypeDeclaration[C](c)(c.typeOf[Boolean])
  }

  def newTypeDeclaration[C <: blackbox.Context](c: C)(tpe: C#Type): TypeDeclaration[C] = {
    new TypeDeclaration[C](c)(tpe)
  }

  def newUnknownTypeDeclaration[C <: blackbox.Context](c: C): TypeDeclaration[C] = {
    new TypeDeclaration[C](c)(c.typeOf[Nothing])
  }

  protected def gatherTypeParameterDeclarations[C <: blackbox.Context](c: C)(tpe: C#Type): mutable.Map[String, Seq[TypeParameterDeclaration[C]]] = {
    val typeParameterDeclarationsMap = mutable.Map[String, Seq[TypeParameterDeclaration[C]]]()
    val typeSymbol = tpe.typeSymbol.asType
    if (typeSymbol == null) return typeParameterDeclarationsMap
    typeParameterDeclarationsMap.put(typeSymbol.fullName , createTypeParameterDeclarations(c)(tpe))
    typeParameterDeclarationsMap
  }

  def createTypeParameterDeclarations[C <: blackbox.Context](c: C)(tpe: C#Type): Seq[TypeParameterDeclaration[C]] = {
    assertNotNull(tpe, c)
    tpe.typeSymbol.typeSignature.typeParams.zip(tpe.typeArgs).map { t =>
      new TypeParameterDeclaration[C](c)(t._1.asInstanceOf[C#Symbol], t._2.asInstanceOf[C#Type])
    }
  }
}
