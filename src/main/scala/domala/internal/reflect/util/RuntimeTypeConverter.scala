...
object RuntimeTypeConverter extends TypeConverter {
  import ru._
  type T = Type
  ...
  override def headParamType(tpe: T): T =
    tpe.members.find(_.isConstructor).get.asMethod.paramLists.flatten.head.typeSignature

  override def typeArgs(tpe: T): List[T] = tpe.typeArgs

  override def isString(tpe: T): Boolean = tpe =:= typeOf[String]

  override def isBoolean(tpe: T): Boolean = tpe =:= typeOf[Boolean]
  ...
  override def isRuntimeEntity(tpe: T): Boolean = !(tpe <:< typeOf[AnyVal]) && tpe <:< typeOf[Product] && {
    ...
    constructor.paramLists.flatten.forall { (p: Symbol) =>
      RuntimeTypeConverter.toType(p.typeSignature) match {
